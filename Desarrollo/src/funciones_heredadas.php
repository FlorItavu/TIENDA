...
function CURP($strCURP, $usuario){    
	...
	historia($usuario,"Consulto el CURP de ".$strCURP.": ".$R);
	return $R;
}
...
function DatosVivienda($IdDelegacion, $Usuario, $DescripcionDeUso, $ConsultaMSSQLSERVER){
    ...
    $url= $URLwebserviceVivienda."/sql.asp?IdDel=0&user=".$Usuario."&token=".$MiToken."&sql=".$sql; // preparamos la URL para el webservice de vivienda
...
function FTP_descargar($archivo){
	...
	//echo $archivo;
	//echo 'va a escribir en local: tmp/'.$archivo;
...
//echo '<span class="hijo">';
		...
		echo '<p>'.$mensaje.'</p>';
		echo '<a class="ui primary button" href="'.$link.'">Aceptar</a>  ';
...
echo '                <div id="beta_buscar">';
  ...
  <div class="ui icon input">
    <input class="prompt" type="text" id="beta_buscar_input" name="busqueda"  style="font-size: 14pt" placeholder="'.$placeholder.'">
